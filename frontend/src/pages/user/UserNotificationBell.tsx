...
const UserNotificationBell = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await Axios({
        method: SummaryApi.my_notifications.method,
        url: SummaryApi.my_notifications.url,
      }); // ✅ user endpoint
      if (res.data.success) setNotifications(res.data.data);
    } catch { console.error("Notification fetch error"); }
  ...
  const fetchUnreadCount = async () => {
    try {
      const res = await Axios({
        method:SummaryApi.my_unread_count.method,
        url:SummaryApi.my_unread_count.url,
      }); // ✅ user endpoint
      if (res.data.success) setUnreadCount(res.data.data.count);
    } catch { console.error("Unread count error"); }
  };
          ...
          {loading ? (
            ...
            notifications.map(n => (
              <div
                ...
                className={`p-3 border-b cursor-pointer transition ${!n.read ? "bg-indigo-50" : "hover:bg-slate-50"}`}
              >
